...
public class ExecC {
	public static void main(String[] args) {
		...
		System.out.println(홍길동.toString()); // 홍길동 전부를 출력
		System.out.println(홍길동.stName); // 홍길동 이름만 출력
		System.out.println(stdList.get(0).stName);
		 ...
		 * 객체로 변환된다
		 * stList.get(1) 은 stdList 의 index 1 의 객체 데이터다
		 */
		System.out.println(stdList.get(1).stName);
		
		for(int i = 0 ; i < stdList.size(); i ++) {
			String stName = stdList.get(i).stName;
			System.out.println(stName); // 홍길동, 성춘향 둘다 출력
		}
